...
UID:${externalID}
SUMMARY:teaaaaa
DTSTART:${date.toFormat('yyyyMMdd')}T${date.toFormat('HHmm')}000Z
DTEND:${date.plus({hour: 1}).toFormat('yyyyMMdd')}T${date.plus({hour: 1}).toFormat('HHmm')}000Z
CLASS:PUBLIC
PRIORITY:5
DTSTAMP:20210402T205602Z
TRANSP:OPAQUE
...
export class calDavEvents implements MigrationInterface {
  public async up(): Promise<{ event: CalDavEventEntity, repeatedEvent: CalDavEventEntity }> {
    // @ts-ignore
